...
export default function WeaknessConceptDrillPage({
  ...
  const handleSkipPractice = useCallback(async () => {
    if (!testSession) {
      return;
    }
    setIsSkipping(true);
    ...
    setSkipMessage("");
    try {
      const response = await fetch("/api/weakness/test/skip", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      const payload = (await response.json()) as SkipWeaknessTestResponse;
      if (!response.ok || !payload.success || !payload.result) {
        throw new Error(payload.message ?? "Unable to skip concept practice test.");
      }
      setSkipMessage("Practice skipped.");
      setSubmitResult(null);
      setTestSession(null);
      setAnswers({});
    } catch (error) {
      setTestError(error instanceof Error ? error.message : "Unable to skip concept practice test.");
    } finally {
      setIsSkipping(false);
    }
  }, [conceptTag, courseId, testSession]);
  ...
  return (
    <section className="mx-auto w-full max-w-6xl px-5 pb-12 pt-28 sm:px-8 sm:pt-32">
      <div className="rounded-3xl border-2 border-[#1F2937]/12 bg-white p-6 shadow-[0_10px_25px_rgba(31,41,55,0.08)] sm:p-8">
        ...
        {skipMessage ? (
          <p className="mt-4 rounded-xl bg-[#eef9e8] px-3 py-2 text-sm font-semibold text-[#2e7d32]">
            {skipMessage}
          </p>
        ) : null}
