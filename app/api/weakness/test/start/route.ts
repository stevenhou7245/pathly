...
export async function POST(request: Request) {
  try {
    ...
    if (rawBody && typeof rawBody === "object" && "action" in (rawBody as Record<string, unknown>)) {
      const actionValue = String((rawBody as Record<string, unknown>).action ?? "").trim();
      console.info("[weakness] test_start:action_ignored", {
        action: actionValue || null,
        reason: "action is routing-only and never mapped to weakness_test_sessions.status",
      });
    }
